...
- **v1.1.25** (2026-09-03) — Flagged 4 sections that have no corresponding sheet in the 9.1.1 workbook: diffing the real 9.1.1 export against 9.1.0 sheet-by-sheet found 5 sheets present in 9.1.0 with zero trace anywhere in 9.1.1 — Active Directory Inputs, Cloud-Based Ransomware Recovery, Cross Cloud Mobility, Private AI Ready Infrastructure, and Public Reference Sheet (a pure computed recap with no app equivalent). Added a page-level `droppedInWorkbook` flag (`core/reference.js`) and an amber advisory banner (`pageMissingFromWorkbook()` in `index.html`) on the 4 corresponding app pages when VCF Version 9.1.1.0 is selected — fields stay fully usable, the banner just notes there's no official 9.1.1 worksheet backing them. The sizing calculator's own Cloud-Based Ransomware Recovery / HCX Connector constants are unaffected (identical between versions in "Static Reference Tables") — only the dedicated planning sheets are gone.
- **v1.1.24** (2026-09-03) — VCF Version is now selectable up front, and drives the Sizing calculator's default: the "VCF Version" field on the VCF & VVF Planning page (the first field of the first section) only ever offered `9.1.0.0`, even after 9.1.1 support landed elsewhere (Excel import in v1.1.22, Sizing calculator in v1.1.23). Added `9.1.1.0` as a second option. The Sizing page's Workbook Version selector now defaults from this global choice on load and whenever it changes (`syncSizingWorkbookVersion()` in `index.html`, called from `init()` and a `$watch('form.vcfVersion', ...)`), so picking 9.1.1.0 up front is the one decision that sets the Sizing calculator's tables correctly — its own selector still works as a local override afterwards.
- **v1.1.23** (2026-09-03) — Made the Management Domain Sizing calculator workbook-version-aware (9.1.0 vs 9.1.1): the built-in sizing calculator always used 9.1.0-calibrated constants, even though Broadcom changed several sizing tables in the 9.1.1 workbook — SSP retiered from a single "Include" tier to Medium/Large/XLarge (with a cheaper Licensing Hub), Cloud Proxy's Medium/Large merged into one "Standard" tier, VCF Operations for Networks gained XL/XXL tiers, and — most significantly — the VCF Services Runtime worker-node formula was replaced entirely: 9.1.0 sized worker nodes from a flat per-size table, 9.1.1 derives node count from a capacity model (a fixed per-instance platform-services baseline plus whichever of Log Management / Real-time Metrics are enabled, independently rounded for RAM and CPU, then taking the larger of the two). Added a **Workbook Version** selector to the Sizing page; `core/data.js` now keeps version-keyed lookup tables (`LT_TABLES['9.1.0']` / `['9.1.1']`) and `core/sizing.js` picks the right one, transcribed and cross-checked directly against the real 9.1.1 workbook's live Excel formulas (not just cached values — see `tools/sizing_scenarios.md` Scenario 8 for what's independently verified vs. formula-derived). `tools/check_lt_constants.py` now verifies both workbook versions.
- **v1.1.22** (2026-09-03) — Added support for the VCF 9.1.1 Planning & Preparation Workbook: Excel import only understood the 9.1.0 workbook's cell layout — dropping in a 9.1.1 export (`v1.9.1.101`) silently imported wrong or blank values, because VMware re-flowed the "Deploy Management Domain" sheet (new IPv6 gateway rows, per-host FQDN rows, a "VCF Automation" section, and more), shifting every mapped field by a different amount rather than a constant offset. `core/excel-import.js` now keeps a separate, independently-calibrated cell map per workbook version family (`IMPORT_MAPS['9.1.0']` / `IMPORT_MAPS['9.1.1']`) and picks the right one automatically from the release number already read off the workbook's own "Version History" sheet — no version picker needed. The Excel Import Report now also shows which field map was used, so a misdetected version is visible immediately rather than producing a silent bad import.
- **v1.1.21** (2026-08-31) — Fixed missing ESXi credentials block in exported hostSpecs: the ESXi root password wasn't appearing in the export at all — `buildHostSpecs()` only attached a `credentials` block when **Skip ESXi SSL Thumbprint Validation** was set to Exclude, on the assumption (documented in the field's own notes) that skipping thumbprint validation meant credentials weren't needed either. That's backwards from what VCF Installer actually wants — the reference export (`Examples/VCF-site1.json`) includes a `credentials` block on every host regardless, and doesn't even set this flag. Host credentials are now always included whenever a password is available, independent of the thumbprint-validation toggle (which only ever controlled thumbprint checking, not credentials).
- **v1.1.20** (2026-08-31) — Fixed generated passwords rejected by VCF Installer's character whitelist: VCF Installer rejected passwords produced by the "🎲 Generate Passwords" button (added in v1.1.17) for NSX Root, VSP system user, and VCF Operations Admin with "Password must contain only letters, numbers and the following special characters" — the generator's special-character pool (`!@#$%^-_`) included `%`, `-`, and `_`, none of which are on VCF Installer's actual whitelists, and was missing `*` which every field accepts. The pool is now `!@#$^*` — the intersection of every whitelist seen so far — so generated passwords are valid everywhere the button is used. Any password generated before this fix should be regenerated with the 🎲 button before re-exporting.
- **v1.1.19** (2026-08-31) — Fixed missing VCF Operations root user password: VCF Installer rejected the export with "Root user password missing for VCF Operations". Each node in `vcfOperationsSpec.nodes[]` needs its own `rootUserPassword` — a separate OS-level credential from the `adminUserPassword` (UI login) already fixed in v1.1.17 — but no form field existed for it and `buildVcfOperationsSpec()` never set it. Added a new **Root User Password** field, applied to every VCF Operations node (Primary/Replica/Data share one root credential from the appliance template).
- **v1.1.18** (2026-08-31) — Fixed SDDC Manager password gating + added a password reveal toggle: another real VCF Installer import failed with "SDDC Manager Local User Password must be at least 12 characters", "SDDC Manager root User Password must be at least 15 characters", and "SDDC Manager vcf User Password must be at least 15 characters" — the v1.1.17 fix wired these fields into the export correctly, but the **Admin Password** and **Root Password** fields were still hidden (and therefore unfillable) whenever SDDC Manager Location was set to "External deployment", on the assumption that mode didn't need them. The real installer proves that assumption wrong — both fields are now always visible and required, regardless of SDDC Manager Location. Also added a **👁 show/hide toggle** next to every password field in the data-entry form, so a typed or 🎲-generated password can actually be read back — previously there was no way to see what value a field held. The As-Built recap page still masks passwords as •••••••• by design.
- **v1.1.17** (2026-08-31) — VCF Installer JSON export: wired up remaining Day-0 appliance passwords + password generator button: following the v1.1.16 fix, auditing NSX turned up the same bug in three more places: `nsxtAdminPassword`/`nsxtAuditPassword`/`rootNsxtManagerPassword` (NSX Manager), `rootPassword`/`sshPassword`/`localUserPassword` (SDDC Manager), `adminUserSsoPassword` (vCenter SSO), and `adminUserPassword` (VCF Operations) all had a form field the user could fill in, but no builder ever read it — the value was silently dropped from the export. All four are now wired in and required regardless of the "Auto-generate passwords" toggle, matching the same Day-0 reasoning as the vCenter root / VSP fix. Added a new **NSX Manager Root Password** field (the schema needed it and no field existed for it). Also added a **🎲 Generate Passwords** button to every section that has password fields — one click fills any empty password field in that section with a random 16-character value guaranteed to contain uppercase, lowercase, digits and special characters with no 3+ repeated characters, satisfying every length/complexity rule in the app without ever overwriting a password already typed.
- **v1.1.16** (2026-08-31) — VCF Installer JSON export: fixed vCenter root / VSP-Cloud Proxy password validation errors: a real VCF Installer import failed with "vCenter Server root password must be between 8 and 20 characters", "VCF Cloud Proxy password must be at least 15 characters", and "VSP password must be at least 15 characters". `buildVcenterSpec()` and `buildVspClusterSpec()` in `index.html` were sending an empty string for `rootVcenterPassword` and `vspClusterSpec.systemUserPassword` whenever "Auto-generate passwords" was selected, on the assumption VCF Installer would generate them at bring-up — it doesn't, for these two fields, since there is no live orchestrator yet to do so before the fleet exists. The **vCenter Root Password** field is now always required (no longer hidden behind the auto-generate toggle), and the existing but previously unused **VSP / Cloud Proxy System User Password** field (`vcfMgmtSvcSshPw`, renamed from "SSH Password") is now wired into the export and made required too. Pre-export validation warns if either is left blank.
- **v1.1.15** (2026-08-30) — VCF Installer JSON export: fixed missing VCF Management Services / License Server / Identity Broker: a real VCF Installer import surfaced 4 errors — missing VCF Management Services IP pool range, Cloud Proxy appliance FQDN, License Server FQDN, and Identity Broker FQDN — even though the form already collected all of that data. `buildVcfInstallerSpec()` in `index.html` simply never read it: **vspClusterSpec**, **licenseServerSpec**, **vidbSpec**, and the **fleetLcmSpec / sddcLcmSpec / fleetDepotSpec / telemetryAcceptorSpec / saltSpec / saltRaasSpec** blocks are now built and included. Added a new **VCF Services Runtime IP Range End** field (the pool needed a start *and* end address; only the start existed). Also added a **VCF Operations Remote Collector** section (on by default, toggle to exclude) producing `vcfOperationsCollectorSpec`, and an **ESXi Root Password** mode toggle — Same for all hosts (unchanged default) vs. Different per host, with 16 new per-host password fields (`m01Host{1-16}Pw`) feeding `buildHostSpecs()`. Pre-export validation now warns if any of these required fields are missing, instead of only failing inside VCF Installer.
